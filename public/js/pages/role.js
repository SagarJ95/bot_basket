...
var table = $("#role_table").DataTable({
  ...
  columns: [
    { data: "role_name", className: "text-center fs-6" },
...
$("form#addEditForm").on("submit", function (e) {
  ...
  var actionUrl = id == "" ? "/api/roles" : "/api/roles/" + id;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status) {
        ...
        $('form#addEditForm [name="role_id"]').val(response.data.id);
        $('form#addEditForm [name="role_name"]').val(response.data.role_name);
      } else {
        toastr.error(response.message);
      }
    },
