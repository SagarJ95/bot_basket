...
const getCustomers = catchAsync(async (req, res) => {
  try {
    ...
    const query = `select c.id,CONCAT(c.first_name,' ',c.last_name) as customer_name,
        ...
        COALESCE(SUM(oi.quantity * oi.price::numeric), 0) AS total_revenue,
        TO_CHAR(MAX(o.created_at), 'FMDDth FMMonth YYYY') AS last_order_date,
        c.status as visibilty_status,c.phone_country_code,c.whatsapp_country_code
        from customers as c
        LEFT JOIN orders as o ON c.id = o.customer_id AND o.status = $1
        LEFT JOIN order_items as oi ON o.id = oi.order_id AND o.status = $2
        where c.deleted_at IS NULL ${searchQuery}
...
const getParticularCustomerInfo = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const query_params = [customer_id, 1];
    ...
    const query = `SELECT
            ...
            ) FILTER (WHERE ca.id IS NOT NULL AND ca.status = $2),
            '[]'
        ) AS addresses
        FROM customers AS c
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message: customerInfo
        ...
        : "Customer created unsucessfully",
    });
  } catch (e) {
    return res.status(200).json({
      status: false,
...
const update_customer_info = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const existing = await db.query(`SELECT * FROM customers WHERE id = $1`, [
      customer_id,
    ]);
    ...
    return res.status(200).json({
      ...
      message:
        updateCustomerPassword.length > 0
          ...
          : "Customer information updated unsuccessfullyy",
    });
  } catch (e) {
    return res.status(200).json({
      status: false,
...
const update_customer_address = catchAsync(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message:
        updateCustomerPassword.length > 0
          ? "Updated customer address info sucessfully"
          : "Updated customer address unsucessfully",
    });
  } catch (e) {
    return res.status(200).json({
      status: false,
...
const activationStatus = catchAsync(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      status: true,
      message:
        updateStatus.rowCount > 0
          ? "Visibilty status Updated successfully"
          : "Visibilty status Updated unsuccessfully",
    });
  } catch (e) {
    return res.status(200).json({
      status: false,
...
const delete_customer_address = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const updateStatus = await db.query(
      `update customer_addresses SET status = $1 , updated_by = $2, deleted_at = NOW() Where customer_id = $3 and id = $4 RETURNING id`,
      [0, req.user.id, customer_id, address_id]
    );
    ...
    return res.status(200).json({
      status: true,
      message: updatedId
        ? "Deleted Customer address successfully"
        : "Deleted Customer address unsuccessfully",
    });
  } catch (e) {
    return res.status(200).json({
      status: false,
  ...
  try {
    const add_address = await db.query(
      `INSERT INTO customer_addresses
         (customer_id, full_name, mobile_number, address1, zip_code, country, city, state,phone_country_code,created_by)
    ...
    if (add_address.rowCount > 0) {
      ...
      res.status(400).json({
        status: false,
        message: "Address added unsuccessfully!",
      });
    }
  } catch (error) {
